function CardGameTask(subjectnum,cbtype)
%cbtype = 1, then A, I, A, I

if cbtype == 1
    feedbackorder = [1 2 1 2];
else
    feedbackorder = [2 1 2 1];
end
...
try
    ...
    for gametype = feedbackorder
        ...
        if gametype == 1
            ...
            type = 'affective';
            if count < 3
                instruction_msg = sprintf('Run %d of %d: Get points in the Point Game!', count, length(feedbackorder));
            else
                instruction_msg = sprintf('Run %d of %d: You need more points to play in the Bonus Game!', count, length(feedbackorder));
            end
            deck1_color = cool1;
            deck2_color = cool2;
            deck3_color = cool3;
        ...
        Screen('DrawText', window, 'Press the spacebar to start the task.', (centerhoriz-(normBoundsRect(3)/2)), (centervert+(70*scale_res(2))), black);
