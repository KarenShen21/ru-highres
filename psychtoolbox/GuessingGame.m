function GuessingGame(subjectnum)
...
ITI_list = [repmat(3,1,23) repmat(4.75,1,12) repmat(6.5,1,6) repmat(8.25,1,3) 10];
ITI_list = Shuffle(ITI_list) + 1;
ISI_list = [repmat(2,1,23) repmat(3.5,1,12) repmat(5,1,6) repmat(6.5,1,3) 8];
...
FB_rect = [CenterSpotHor-35 CenterSpotVert-35 CenterSpotHor+35 CenterSpotVert+35];
    ...
    while keep_going
        ...
        longest_msg = 'Index Finger                         Middle Finger'; %You''ll see the outcome of one decision at the end of the task.
        ...
        while ~press
            ...
            if find(responsecode) == L_arrow %LEFT
                ...
                press = 1;
                Screen('Flip', window);
            elseif find(responsecode) == R_arrow %Middle --> different from other task
